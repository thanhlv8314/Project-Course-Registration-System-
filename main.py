...
            elif sub == '2':
                courses = c_repo.get_all_courses()

                if not courses:
                    print("❌ Chưa có môn học nào. Hãy thêm môn trước.")
                else:
                    print("\n--- DANH SÁCH MÔN HỌC ---")
                    for c in courses:
                        print(f"{c['course_code']} - {c['course_name']} ({c['credits']} tín chỉ)")

                    course_code = input("\nNhập MÃ MÔN để mở lớp (0 hủy): ").strip()
                    if course_code != '0':
...
                print("1. MỞ cổng đăng ký")
                print("2. ĐÓNG cổng đăng ký")
                print("0. Quay lại")

                opt = input("Chọn: ").strip()

                if opt == '1':
                    print(c_repo.set_registration_status('1'))

                elif opt == '2':
                    print(c_repo.set_registration_status('0'))
...
    # ========= UC16 =========
        elif choice == '6':
            data = o_repo.generate_report()

            if not data:
                print("❌ Chưa có dữ liệu thống kê")
            else:
                total_courses = len(data)
                total_students = sum(r['total_students'] for r in data)

                print("\n=== BÁO CÁO ĐĂNG KÝ HỌC PHẦN ===")
                print(f"Tổng số môn học: {total_courses}")
                print(f"Tổng số sinh viên đăng ký: {total_students}\n")

                print("Chi tiết theo môn:")
                for r in data:
                    print(f"- {r['course_name']}: {r['total_students']} sinh viên")

            input("\nEnter để tiếp tục...")
        elif choice == '0':
            print("Đã đăng xuất quản trị viên")
            return
...
# 1. Tra cứu môn
# ===== UC01: Tra cứu môn học =====
        if c == '1':
            while True:
                print("\n--- TRA CỨU MÔN HỌC ---")
                print("1. Xem toàn bộ danh sách môn")
                print("2. Tìm môn theo mã / tên")
                print("0. Quay lại")
...
                elif sub == '2':
                    keyword = input("Nhập mã hoặc tên môn: ").strip()
                    courses = c_repo.search_courses(keyword)
...
                print("-" * 60)
                if not courses:
                    print("Không có môn học phù hợp")
                else:
                    for i in courses:
                        print(f"{i['course_code']} - {i['course_name']} - {i['credits']} tín chỉ")

        # 2. Đăng ký
        elif c == '2':
            if not c_repo.is_registration_open():
                print("❌ Hiện tại KHÔNG trong thời gian đăng ký học phần")
                input("Enter để quay lại...")
                continue
